...
The optional logger uses one W&B run per study, layout and learner seed, with separate `control/` and `design/` agent sections and shared `eval/` outcomes. Runs are classified as `job_type=control`; configuration explicitly identifies the active control agent and inactive fixed-catalogue design agent. Its isolated SDK leaves the scientific environment and authoritative local records unchanged:
...
Both agent sections declare the same scalar fields: round and measured-step progress, update count, active/status/completion/failure state, raw and normalized reward, policy/value losses, learning rate, entropy and entropy fraction, approximate/exact KL, clipping fraction, and scalar gradient/update norms. The catalogue does not train a design agent: `design/active=false`, `design/status=inactive`, and all its remaining fields are null, including completion. Control metrics also start empty until observed. These empty fields are emitted only once, not repeated every round; no extra design run, synthetic zero curve, tensor histogram, or per-trial run is created. W&B omits null values from its summary/charts, so the inactive design agent is visible through its status and configuration, not a fabricated learning curve. This classification does not enable design training or import historical joint-training runs.
